...
```mermaid
flowchart TD
    A[LEO 卫星 n 的轨道位置] --> B[卫星到用户距离 d_n[t]]
    B --> C[大尺度衰落与 Rician 信道]
    C --> D[imperfect CSI 与 orbit information]
    D --> E[本地 FL 模型选择 beamforming codebook]
    E --> F[多颗卫星分布式发射]
    F --> G[地面用户接收相干叠加信号]
    G --> H[平均可达速率评估]
```
...
```mermaid
flowchart TD
    A[收集本地轨道与信道相关样本] --> B[卫星本地训练 ResNet-18]
    B --> C[上传模型更新而非原始数据]
    C --> D[联邦聚合得到全局模型]
    D --> E[下发更新后的模型参数]
    E --> F[基于 imperfect CSI 预测后续波束]
    F --> G[执行 codebook-based distributed beamforming]
    G --> H[比较可达速率与学习性能]
```
